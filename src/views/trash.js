...
import { saveAll } from "../store/database.js";
...
export function renderTrash() {
  ...
  db.trash
    ...
    .forEach((item) => {
      ...
      const detail =
        data.name || (data.no ? "Bill #" + data.no : "") || (data.amount ? money(data.amount) : "") || "—";
      html += `<tr>
...
export function handleRestore(trashId) {
  restoreFromTrash(trashId);
  saveAll();
  refresh();
}

export function handlePurge(trashId) {
  purgeFromTrash(trashId);
  refresh();
}
